...
pub fn bn_rand(size: usize) -> Result<BigNumber, IndyCryptoError> {
    if MockHelper::is_injected() {
        return match size {
            1021 => Ok(BigNumber::from_dec("").unwrap()),
            1022 => Ok(BigNumber::from_dec("").unwrap()),
            1023 => Ok(BigNumber::from_dec("").unwrap()),
            ...
            894 => Ok(BigNumber::from_dec("").unwrap()),
            895 => Ok(BigNumber::from_dec("").unwrap()),
            896 => Ok(BigNumber::from_dec("").unwrap()),
            897 => Ok(BigNumber::from_dec("").unwrap()),
            _ => {
...
//        println!("p_5={:?}", p_5);
    ...
    fn test_check_public_ps() {
        ...
        assert!(p_4.is_prime(Some(&mut ctx)).unwrap());
        assert!(p_5.is_safe_prime(Some(&mut ctx)).unwrap());
        assert_eq!(p_1, p_0.lshift1().unwrap().increment().unwrap());
        assert_eq!(p_2, p_1.lshift1().unwrap().increment().unwrap());
        assert_eq!(p_3, p_2.lshift1().unwrap().increment().unwrap());
        assert_eq!(p_5, p_4.lshift1().unwrap().increment().unwrap());
    }
